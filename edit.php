...
function edit_user(&$sqlr, &$sqlc)
{
	...
	$referred_by = $sqlr->result($sqlr->query('
		SELECT username 
		FROM account 
		WHERE id = \''.$refguid.'\''), 0, 'username');
unset($refguid);

	if ($acc = $sqlc->fetch_assoc($sqlr->query('
		SELECT email, gmlevel, joindate, expansion, last_ip 
		FROM account 
...
$output .= '
	...
	<tr>
		...
		<td align="right">
			...
			if (is_dir('./core/themes'))
			{
				if ($dh = opendir('./core/themes'))
				{
					while (($file = readdir($dh)) == true)
					{
...
require_once 'libs/valid_lib.php';
	...
	$sqlr->query('
		UPDATE account 
		SET email = \''.$new_mail.'\', '.$new_pass.' v=0, s=0, expansion = \''.$new_expansion.'\' 
		WHERE username = \''.$user_name.'\'');
	if (doupdate_referral($referredby, $sqlr) || $sqlr->affected_rows())
...
function doupdate_referral($referredby, &$sqlr)
{
